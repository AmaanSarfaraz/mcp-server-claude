  ...
  serve(
    {
      fetch: app.fetch,
      port,
      hostname: '0.0.0.0', // Add this line - bind to all interfaces
    },
    (info) => {
      console.error(`MCP StreamableHTTP Server running at http://0.0.0.0:${info.port}`);
      console.error(`- MCP Endpoint: http://0.0.0.0:${info.port}/mcp`);
      console.error(`- Health Check: http://0.0.0.0:${info.port}/health`);
    }
  );
